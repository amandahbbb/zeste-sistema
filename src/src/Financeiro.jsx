...
const CATS=['HONORÁRIOS','SERV. TERCEIROS','REPASSE / PARCEIRO','SOFTWARES','DESP. ADMINISTRATIVAS','MARKETING','PRÓ-LABORE','INVESTIMENTO','OUTROS'];
...
const CAT_NAT={'HONORÁRIOS':'DESPESA FIXA','SERV. TERCEIROS':'DESPESA VARIÁVEL','REPASSE / PARCEIRO':'DESPESA VARIÁVEL','SOFTWARES':'DESPESA FIXA','DESP. ADMINISTRATIVAS':'DESPESA FIXA','MARKETING':'DESPESA VARIÁVEL','PRÓ-LABORE':'DESPESA FIXA','INVESTIMENTO':'INVESTIMENTO','OUTROS':'DESPESA FIXA'};
const SUBS={'HONORÁRIOS':['CONSULTORIA OPERACIONAL','CONSULTORIA GERENCIAL','PRODUÇÃO DE MATERIAIS','TREINAMENTO / MENTORIA','DIAGNÓSTICO'],'SERV. TERCEIROS':['DESIGN / DIAGRAMAÇÃO','FOTOGRAFIA','PROGRAMAÇÃO','FREELANCER'],'REPASSE / PARCEIRO':['PARCEIRO DE PROJETO','FREELANCER','OUTROS'],'SOFTWARES':['ASSINATURA','DOMÍNIO / HOSPEDAGEM','OUTROS'],'DESP. ADMINISTRATIVAS':['HONORÁRIOS CONTADOR','TELEFONE / INTERNET','MATERIAL DE ESCRITÓRIO','IMPOSTOS / TAXAS','OUTRAS'],'MARKETING':['IMPRESSÃO / MATERIAIS','REDES SOCIAIS','OUTROS'],'PRÓ-LABORE':['RETIRADA'],'INVESTIMENTO':['IDENTIDADE VISUAL / BRANDING','SISTEMAS / SOFTWARE','EQUIPAMENTOS','CURSOS / CAPACITAÇÃO','OUTROS'],'OUTROS':['OUTROS']};
...
function Anexos({anexos=[],onChange}){const ref=useRef();const[prev,setPrev]=useState(null);const add=async(files)=>{const p=await Promise.all(Array.from(files).map(compressImg));onChange([...anexos,...p]);};return(<div><div className="att-grid">{anexos.map((a,i)=>(<div key={i} style={{position:'relative'}}>{a.type==='image'?<img src={a.data} className="att-thumb" alt="" onClick={()=>setPrev(a)}/>:<div className="att-pdf" onClick={()=>setPrev(a)}><span style={{fontSize:22}}>📄</span><span style={{fontSize:9,color:'var(--cinzaE)'}}>{(a.name||'').slice(0,10)}</span></div>}<button onClick={()=>onChange(anexos.filter((_,j)=>j!==i))} style={{position:'absolute',top:-6,right:-6,width:20,height:20,borderRadius:'50%',background:'var(--coral)',color:'var(--branco)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button></div>))}<div className="att-add" onClick={()=>ref.current.click()}><span style={{fontSize:22}}>📎</span><span>Anexo</span></div></div><input ref={ref} type="file" accept="image/*,application/pdf" multiple onChange={e=>add(e.target.files)}/>{prev&&(<Modal title={prev.name||'Comprovante'} onClose={()=>setPrev(null)}>{prev.type==='image'?<img src={prev.data} style={{width:'100%',borderRadius:8}} alt=""/>:<div style={{textAlign:'center',padding:32}}><span style={{fontSize:48}}>📄</span><p style={{marginTop:10,color:'var(--cinzaE)',marginBottom:14}}>{prev.name}</p><a href={prev.data} download={prev.name||'comprovante.pdf'} style={{color:'var(--azul)',fontWeight:600}}>⬇ Baixar PDF</a></div>}</Modal>)}</div>);}
