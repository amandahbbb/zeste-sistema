...
.ger-sec{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:6px 24px 18px}.ger-fotos{display:flex;gap:12px;justify-content:center;align-items:center;margin:16px 24px 8px;flex-wrap:wrap}.ger-foto{width:70mm;height:70mm;background-size:contain;background-repeat:no-repeat;background-position:center;border-radius:12px;background-color:#fff;border:1px solid #ecebe4}.ger-foto.dupla{width:58mm;height:58mm}
...
@media print{body{background:#fff}
  ...
  .card-prato,.receita,.card-ger,.prato-head,.ger-head,.kpi,.capa,.ger-foto,.ger-fotos{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
...
function _fotoBox(u, u2) {
  const urls = [_fotoUrl(u), _fotoUrl(u2)].filter(Boolean);
  if (!urls.length) return "";
  const cls = urls.length > 1 ? "dupla" : "";
  const imgs = urls.map(x => `<div class="ger-foto ${cls}" style="background-image:url('${x}')"></div>`).join("");
  return `<div class="ger-fotos">${imgs}</div>`;
}
