...
async function sbLoad(table,t){try{const r=await fetch(`${SB_URL}/rest/v1/${table}?deleted_at=is.null&order=created_at.desc`,{headers:sbH(t)});if(!r.ok)return null;const d=await r.json();return Array.isArray(d)?d.map(r=>({...r.dados,_id:r.id,_cliente:r.cliente_id,_criadoEm:r.created_at})):null;}catch{return null;}}
...
function PratoForm({open,prato,onClose,onSave,onDelete,ingredientes,fichasCalc,souCli,ehAdmin,token}){
  ...
  return(<Modal title={prato?'Editar Prato':'Novo Prato'} onClose={onClose}>
    ...
    <div className="ft-fld" style={{marginTop:12}}><label className="ft-flbl">Empratamento / modo (opcional)</label><textarea rows={10} value={p.modoPreparo||''} onChange={e=>setP(pr=>({...pr,modoPreparo:e.target.value}))} style={{resize:'vertical',height:240,minHeight:180,width:'100%',boxSizing:'border-box',lineHeight:1.5}} placeholder="1. Empratar…"/></div>
  ...
  return(<div className="ft-page">
    ...
    <div className="ft-pc"><div className="ft-card">
      ...
      {filtered.map(i=>(<div key={i.id} className="ft-row" onClick={()=>setEdit({...i})}>
        <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{i.nome}</div><div style={{fontSize:11,color:'var(--cinzaE)',marginTop:2}}>{i.un} · FC {num(i.fc)} · FK {num(i.fk)}{i._cliente?` · ${({zeste:'Zeste','440':'440 Restaurante','440-conf':'440 Confeitaria','mimo-baby':'Mimo Baby'}[i._cliente]||i._cliente)}`:''}{i._criadoEm?` · ${new Date(i._criadoEm).toLocaleDateString('pt-BR')}`:''}</div></div>
  ...
  return(<div style={{marginTop:14,borderTop:'1px solid var(--cinzaF)',paddingTop:12}}>
    ...
    {addNovo&&<div style={{background:'var(--cinzaF)',borderRadius:8,padding:10,marginTop:6}}>
      {!novoForn?<>
        ...
        <button type="button" onClick={()=>setNf(p=>{const nx={...p,_porPacote:!p._porPacote,_compraPeso:p._compraPeso||55,unidade:!p._porPacote?'KG':p.unidade};const pc=+String(nx._compraPreco).replace(',','.')||0,q=+nx._compraQtd||0,pu=+nx._compraPeso||0;if(nx._porPacote&&pc>0&&q>0&&pu>0)nx.preco=(pc/(q*pu/1000)).toFixed(2);return nx;})} style={{fontSize:11,fontWeight:700,padding:'5px 11px',borderRadius:20,border:'1.5px solid '+(nf._porPacote?'var(--verde)':'var(--cinzaM)'),background:nf._porPacote?'var(--verde)':'#fff',color:nf._porPacote?'#fff':'var(--cinzaE)',cursor:'pointer',marginBottom:8}}>{nf._porPacote?'✓ Preço por pacote/unidade':'⚖ Calcular a partir da compra'}</button>
        {nf._porPacote&&<div style={{display:'flex',gap:6,marginBottom:8,flexWrap:'wrap'}}>
          <div style={{flex:'1 1 88px'}}><div style={{fontSize:9,color:'var(--cinzaE)',fontWeight:700,marginBottom:2}}>COMPRA (R$)</div><input type="text" inputMode="decimal" value={nf._compraPreco??''} onChange={e=>setNf(p=>{const nx={...p,_compraPreco:e.target.value.replace(/[^0-9.,]/g,'')};const pc=+String(nx._compraPreco).replace(',','.')||0,q=+nx._compraQtd||0,pu=+nx._compraPeso||0;if(pc>0&&q>0&&pu>0)nx.preco=(pc/(q*pu/1000)).toFixed(2);return nx;})} placeholder="12,90" style={{width:'100%',boxSizing:'border-box',border:'1.5px solid var(--cinzaM)',borderRadius:7,padding:'8px',fontSize:14}}/></div>
          <div style={{flex:'1 1 60px'}}><div style={{fontSize:9,color:'var(--cinzaE)',fontWeight:700,marginBottom:2}}>QTD</div><input type="text" inputMode="numeric" value={nf._compraQtd??''} onChange={e=>setNf(p=>{const nx={...p,_compraQtd:e.target.value.replace(/[^0-9]/g,'')};const pc=+String(nx._compraPreco).replace(',','.')||0,q=+nx._compraQtd||0,pu=+nx._compraPeso||0;if(pc>0&&q>0&&pu>0)nx.preco=(pc/(q*pu/1000)).toFixed(2);return nx;})} placeholder="30" style={{width:'100%',boxSizing:'border-box',border:'1.5px solid var(--cinzaM)',borderRadius:7,padding:'8px',fontSize:14}}/></div>
          <div style={{flex:'1 1 78px'}}><div style={{fontSize:9,color:'var(--cinzaE)',fontWeight:700,marginBottom:2}}>PESO/UN (g)</div><input type="text" inputMode="numeric" value={nf._compraPeso??''} onChange={e=>setNf(p=>{const nx={...p,_compraPeso:e.target.value.replace(/[^0-9]/g,'')};const pc=+String(nx._compraPreco).replace(',','.')||0,q=+nx._compraQtd||0,pu=+nx._compraPeso||0;if(pc>0&&q>0&&pu>0)nx.preco=(pc/(q*pu/1000)).toFixed(2);return nx;})} placeholder="55" style={{width:'100%',boxSizing:'border-box',border:'1.5px solid var(--cinzaM)',borderRadius:7,padding:'8px',fontSize:14}}/></div>
          <div style={{flex:'1 1 100%',fontSize:11,color:'var(--cinzaE)'}}>Preço/kg: <b style={{color:'var(--verde)'}}>{nf.preco?('R$ '+nf.preco+'/kg'):'—'}</b></div>
        </div>}
