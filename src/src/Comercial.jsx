...
const STATUS_PEDIDO = { "Rascunho": "#8A8A82", "Aguardando": C.cinzaE, "Confirmado": C.azul, "Em trânsito": C.lima, "Entregue": C.verde, "Cancelado": C.coral };
          ...
          return (
            <div key={p.id} className="cmp-row" onClick={() => setModal(p)}>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                ...
                {p.whatsapp && <button onClick={e => { e.stopPropagation(); const n=(p.whatsapp||"").replace(/\D/g,""); const t=encodeURIComponent(`Olá! Pedido Zeste:\n\n${p.itens}\n\nPode confirmar disponibilidade e valor? Obrigada!`); window.open(n?`https://wa.me/55${n}?text=${t}`:`https://wa.me/?text=${t}`,"_blank"); }} style={{ background: "none", border: "none", color: "#25D366", cursor: "pointer", fontSize: 13, marginTop: 4, marginRight: 8 }}>📲</button>}
