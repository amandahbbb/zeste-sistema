...
export default function PortalAdmin({ onBack, token }) {
  ...
  const saveMeta = async (val) => { const m = Math.max(0.05, Math.min(0.9, (+val || 30) / 100)); setSel(s => ({ ...s, cmv_meta: m })); setClientes(cs => cs.map(c => c.cliente_id === sel.cliente_id ? { ...c, cmv_meta: m } : c)); await fetch(`${SB_URL}/rest/v1/fin_portal_clientes?cliente_id=eq.${sel.cliente_id}`, { method: "PATCH", headers: sbH(token), body: JSON.stringify({ cmv_meta: m }) }); };
  ...
  return (
    <div className="pa-wrap">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "8px 16px", background: "#161613", borderBottom: "1px solid #2A2A2A" }}><span style={{ fontSize: 12, color: "#bbb" }}>Meta de CMV do cliente (selo "dentro/acima da meta" nos pratos)</span><span style={{ display: "flex", alignItems: "center", gap: 6 }}><input type="number" min="5" max="90" step="1" defaultValue={Math.round(((sel.cmv_meta || 0.30) * 100))} onBlur={e => saveMeta(e.target.value)} style={{ width: 62, textAlign: "right", background: "#0E0E0C", color: "#fff", border: "1.5px solid #555", borderRadius: 8, padding: "4px 8px", fontSize: 13, fontWeight: 700 }} /><span style={{ fontSize: 12, color: "#bbb" }}>%</span></span></div>
